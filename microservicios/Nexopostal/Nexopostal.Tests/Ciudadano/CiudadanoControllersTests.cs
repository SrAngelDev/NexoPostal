...
public class AdminClientesControllerTests
{
    ...
    public async Task PerfilCompleto_ConPerfilYEnvios_AgregaEstadisticas()
    {
        ...
        var envios = new List<Envio>
        {
            ...
            new()
            {
                NumeroSeguimiento = "NXP-3",
                Pagado = true,
                EstadoActual = EstadoEnvio.Incidencia,
                CosteCalculado = 7m
            }
        };
...
public class AdminEnviosControllerTests
{
    ...
    {
        _ctrl = new AdminEnviosController(Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_svc.Object));
    ...
    {
        _svc.Setup(s => s.ListarAsync(null, null, null, null, null, null, null, 500)).ReturnsAsync(new List<AdminEnvioListItemDto> { new() { NumeroSeguimiento = "NXP-1" } });
    ...
    {
        _svc.Setup(s => s.CambiarEstadoAsync("NXP-1", It.IsAny<CambiarEstadoEnvioDto>(), "admin-1")).ReturnsAsync((new AdminEnvioDetalleDto { NumeroSeguimiento = "NXP-1" }, (string? )null));
    ...
    {
        _svc.Setup(s => s.CambiarEstadoAsync(It.IsAny<string>(), It.IsAny<CambiarEstadoEnvioDto>(), It.IsAny<string?>())).ReturnsAsync((null, "Envío no encontrado"));
    ...
    {
        _svc.Setup(s => s.CambiarEstadoAsync(It.IsAny<string>(), It.IsAny<CambiarEstadoEnvioDto>(), It.IsAny<string?>())).ReturnsAsync((null, "Estado inválido"));
        var r = await _ctrl.CambiarEstado("NXP-1", new CambiarEstadoEnvioDto());
    ...
    {
        _svc.SetupSequence(s => s.AnularAsync(It.IsAny<string>(), It.IsAny<AccionEnvioDto>(), It.IsAny<string?>())).ReturnsAsync((new AdminEnvioDetalleDto { NumeroSeguimiento = "NXP-1" }, null)).ReturnsAsync((null, "Envío no encontrado")).ReturnsAsync((null, "Ya anulado"));
    ...
    {
        _svc.SetupSequence(s => s.ReabrirAsync(It.IsAny<string>(), It.IsAny<AccionEnvioDto>(), It.IsAny<string?>())).ReturnsAsync((new AdminEnvioDetalleDto { NumeroSeguimiento = "NXP-1" }, null)).ReturnsAsync((null, "Envío no encontrado")).ReturnsAsync((null, "No se puede reabrir"));
...
public class PerfilControllerTests
{
    ...
    {
        _ctrl = new PerfilController(Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_repo.Object), NullLogger<PerfilController>.Instance);
    ...
    {
        var c = new PerfilController(Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_repo.Object), NullLogger<PerfilController>.Instance);
    ...
    {
        _repo.Setup(r => r.GetByUserIdAsync("user-1")).ReturnsAsync(new ClientePerfil { IdentityUserId = "user-1", DNI = "X", FechaCreacion = DateTime.UtcNow });
    ...
    {
        _repo.Setup(r => r.GetByUserIdAsync("user-1")).ReturnsAsync((ClientePerfil? )null);
        _repo.Setup(r => r.CreateOrUpdateAsync(It.IsAny<ClientePerfil>())).ReturnsAsync((ClientePerfil p) =>
        {
            p.Id = 99;
            return p;
        });
    ...
    {
        _repo.Setup(r => r.GetByUserIdAsync("user-1")).ReturnsAsync(new ClientePerfil { Id = 1, IdentityUserId = "user-1" });
    ...
    {
        _repo.Setup(r => r.GetByUserIdAsync("user-1")).ReturnsAsync(new ClientePerfil { Id = 1, IdentityUserId = "user-1" });
        _repo.Setup(r => r.GetDireccionByIdAsync(7, 1)).ReturnsAsync(new DireccionFavorita { Id = 7, ClientePerfilId = 1, Alias = "a", NombreDestinatario = "b", Direccion = "c", CodigoPostal = "28001", Ciudad = "d", Provincia = "e" });
        var dto = new CrearDireccionFavoritaDto
        {
            Alias = "X",
            NombreDestinatario = "Y",
