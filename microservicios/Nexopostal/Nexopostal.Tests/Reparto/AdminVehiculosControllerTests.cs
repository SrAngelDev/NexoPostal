...
public class AdminVehiculosControllerTests
{
    ...
    {
        var ctrl = new AdminVehiculosController(Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_svc.Object));
    ...
    {
        _svc.Setup(s => s.ListarAsync(false, null, null)).ReturnsAsync(new List<Vehiculo> { Sample(1), Sample(2) });
    ...
    public async Task Crear_OK_DevuelveCreated()
    {
        ...
        _svc.Setup(s => s.CrearAsync(dto, "admin-1")).ReturnsAsync((Sample(5), null));
    ...
    public async Task Crear_Conflict_CuandoError()
    {
        ...
        _svc.Setup(s => s.CrearAsync(dto, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Matrícula duplicada"));
        var res = await CreateCtrl().Crear(dto);
    ...
    {
        var dto = new ActualizarVehiculoDto
        {
            Matricula = "X",
            Tipo = TipoVehiculo.Furgoneta
        };
        _svc.Setup(s => s.ActualizarAsync(99, dto, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Vehículo no encontrado"));
    ...
    public async Task Actualizar_Conflict()
    {
        ...
        _svc.Setup(s => s.ActualizarAsync(1, dto, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Otro error"));
    ...
    public async Task Actualizar_OK()
    {
        ...
        _svc.Setup(s => s.ActualizarAsync(1, dto, "admin-1")).ReturnsAsync((Sample(1), null));
        var res = await CreateCtrl().Actualizar(1, dto);
        res.Result.Should().BeOfType<OkObjectResult>();
    }
    ...
    {
        _svc.Setup(s => s.AsignarAsync(99, 5, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Vehículo no encontrado"));
    ...
    {
        _svc.Setup(s => s.AsignarAsync(1, 999, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Repartidor no encontrado"));
    ...
    {
        _svc.Setup(s => s.AsignarAsync(1, 5, "admin-1")).ReturnsAsync(((Vehiculo? )null, "Conflicto"));
    ...
    {
        _svc.Setup(s => s.AsignarAsync(1, 5, "admin-1")).ReturnsAsync((Sample(1), null));
        var res = await CreateCtrl().Asignar(1, new AsignarVehiculoDto { RepartidorId = 5 });
        res.Result.Should().BeOfType<OkObjectResult>();
    }
    ...
    {
        _svc.Setup(s => s.ImportarDesdeRepartidoresAsync("admin-1")).ReturnsAsync(new ImportarDesdeRepartidoresResultDto { Importados = 3 });
        var res = await CreateCtrl().Importar();
        var ok = res.Result as OkObjectResult;
        ok!.Value.Should().BeOfType<ImportarDesdeRepartidoresResultDto>().Which.Importados.Should().Be(3);
    }
}
