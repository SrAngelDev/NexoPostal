...
public class CtasControllerTests
{
    ...
    {
        var c = new CtasController(Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_clasif.Object, _adminCta.Object));
        c.WireUser();
        return c;
    }
    ...
    {
        _clasif.Setup(s => s.ObtenerTodosCtas()).ReturnsAsync(new List<CtaResumenDto> { new() { Id = 1, Codigo = "CTA-MAD" } });
    ...
    {
        _clasif.Setup(s => s.ResolverCtaDestino("28001")).ReturnsAsync(new ResolverCtaResponseDto { CodigoPostal = "28001", CtaCodigo = "CTA-MAD" });
...
public class OficinasPostalesControllerTests
{
    ...
    {
        var c = new OficinasPostalesController(_svc.Object, Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_svc.Object));
    ...
    {
        _svc.Setup(s => s.ObtenerTodas()).Returns(new List<OficinaJsonDto> { new() { Id = 1, Nombre = "X" } });
    ...
    {
        _svc.Setup(s => s.BuscarPorCodigoPostal("28001")).Returns(new List<OficinaJsonDto> { new() { Id = 1 } });
    ...
    {
        _svc.Setup(s => s.BuscarPorTexto("madrid")).Returns(new List<OficinaJsonDto> { new() { Id = 1 } });
    ...
    {
        _svc.Setup(s => s.ResolverOficinaPorCp("9999")).ReturnsAsync((ResolverOficinaCtaResponseDto? )null);
    ...
    {
        _svc.Setup(s => s.ResolverOficinaPorCp("28001")).ReturnsAsync(new ResolverOficinaCtaResponseDto());
    ...
    {
        _svc.Setup(s => s.ObtenerOperariosOficina(1)).ReturnsAsync(new List<OperarioOficinaResumenDto>());
    ...
    {
        _svc.Setup(s => s.ObtenerOficinasPorCta(1)).ReturnsAsync(new List<OficinaJsonDto>());
...
public class ScanControllerTests
{
    ...
    {
        var c = new ScanController(NullLogger<ScanController>.Instance, Nexopostal.Tests.Cqrs.CqrsTestServices.CreateSender(_processor.Object));
    ...
    {
        var res = await CreateCtrl().ProcesarEscaneo(new ScanRequestDto { CodigoEscaneado = "NXI-123", ModoOperacion = "modo_invalido" });
        res.Should().BeOfType<BadRequestObjectResult>();
    }
    ...
    public async Task ProcesarEscaneo_Forbid_CuandoRolNoCorresponde()
    {
        ...
        var res = await CreateCtrl(role: "OperarioOficina").ProcesarEscaneo(new ScanRequestDto { CodigoEscaneado = "NXI-123", ModoOperacion = ModosEscaneo.RecepcionCta });
    ...
    {
        _processor.Setup(p => p.ProcesarEscaneo(It.IsAny<ScanRequestDto>())).ReturnsAsync(new ScanResultDto { Exito = true, EstadoNuevo = "EnCta" });
        var res = await CreateCtrl().ProcesarEscaneo(new ScanRequestDto { CodigoEscaneado = "NXI-123", ModoOperacion = ModosEscaneo.RecepcionCta });
        res.Should().BeOfType<OkObjectResult>();
    }
    ...
    {
        _processor.Setup(p => p.ProcesarEscaneo(It.IsAny<ScanRequestDto>())).ReturnsAsync(new ScanResultDto { Exito = false, Mensaje = "no encontrado" });
        var res = await CreateCtrl().ProcesarEscaneo(new ScanRequestDto { CodigoEscaneado = "NXI-123", ModoOperacion = ModosEscaneo.RecepcionCta });
    ...
    {
        var res = await CreateCtrl().ProcesarLote(new ScanBatchRequestDto { CodigosEscaneados = new(), ModoOperacion = ModosEscaneo.RecepcionCta });
        res.Should().BeOfType<BadRequestObjectResult>();
    }
    ...
    {
        var res = await CreateCtrl().ProcesarLote(new ScanBatchRequestDto { CodigosEscaneados = new() { "A" }, ModoOperacion = "no_existe" });
        res.Should().BeOfType<BadRequestObjectResult>();
    }
    ...
    {
        var res = await CreateCtrl(role: "OperarioOficina").ProcesarLote(new ScanBatchRequestDto { CodigosEscaneados = new() { "A" }, ModoOperacion = ModosEscaneo.Clasificacion });
        res.Should().BeOfType<ForbidResult>();
    }
    ...
    {
        _processor.Setup(p => p.ProcesarLote(It.IsAny<ScanBatchRequestDto>())).ReturnsAsync(new ScanBatchResultDto { TotalEscaneados = 1, Exitosos = 1 });
        var res = await CreateCtrl().ProcesarLote(new ScanBatchRequestDto { CodigosEscaneados = new() { "A" }, ModoOperacion = ModosEscaneo.RecepcionCta });
        res.Should().BeOfType<OkObjectResult>();
    }
