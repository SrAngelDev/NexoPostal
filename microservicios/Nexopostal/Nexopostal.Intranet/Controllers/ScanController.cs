...
public class ScanController : ControllerBase
{
    ...
    public ScanController(ILogger<ScanController> logger, ISender sender)
    {
        _logger = logger;
        _sender = sender;
    }
    ...
    public async Task<IActionResult> ProcesarEscaneo([FromBody] ScanRequestDto request)
    {
        ...
        if (!EsModoPermitidoParaRol(request.ModoOperacion, rolUsuario))
            return Forbid();
        var resultado = await _sender.Send(new Nexopostal.Intranet.Application.ScanProcessor.ProcesarEscaneoCommand(request), HttpContext?.RequestAborted ?? CancellationToken.None);
        if (!resultado.Exito)
        {
            _logger.LogWarning("Escaneo fallido: {Codigo} · {Mensaje}", request.CodigoEscaneado, resultado.Mensaje);
            return UnprocessableEntity(resultado);
        }

        _logger.LogInformation("Escaneo exitoso: {Codigo} → {Estado}", request.CodigoEscaneado, resultado.EstadoNuevo);
        return Ok(resultado);
    }
    ...
    public async Task<IActionResult> ProcesarLote([FromBody] ScanBatchRequestDto request)
    {
        ...
        if (!EsModoPermitidoParaRol(request.ModoOperacion, rolUsuario))
            return Forbid();
        var resultado = await _sender.Send(new Nexopostal.Intranet.Application.ScanProcessor.ProcesarLoteCommand(request), HttpContext?.RequestAborted ?? CancellationToken.None);
        _logger.LogInformation("Lote procesado: {Total} total, {Ok} exitosos, {Fail} fallidos", resultado.TotalEscaneados, resultado.Exitosos, resultado.Fallidos);
        return Ok(resultado);
    }
    ...
    public IActionResult ObtenerModos()
    {
        ...
        var todosModos = new[]
        {
            new
            {
                valor = ModosEscaneo.RecepcionOficina,
                etiqueta = "Recepción en oficina",
                icono = "store",
                requiere = "oficina"
            },
            new
            {
                valor = ModosEscaneo.SalidaOficinaACta,
                etiqueta = "Salida hacia CTA origen",
                icono = "local_shipping",
                requiere = "oficina"
            },
            new
            {
                valor = ModosEscaneo.RecepcionCta,
                etiqueta = "Recepción en CTA",
                icono = "warehouse",
                requiere = "cta"
            },
            new
            {
                valor = ModosEscaneo.Clasificacion,
            ...
            new
            {
                valor = ModosEscaneo.DespachoTroncal,
                etiqueta = "Despacho troncal",
                icono = "local_shipping",
                requiere = "cta"
            },
            new
            {
                valor = ModosEscaneo.RecepcionTroncal,
                etiqueta = "Recepción troncal",
                icono = "move_to_inbox",
                requiere = "cta"
            },
            new
            {
                valor = ModosEscaneo.DisponibleParaReparto,
                etiqueta = "Disponible para reparto",
                icono = "outbox",
                requiere = "cta"
            },
            new
            {
                valor = ModosEscaneo.EntregaOficinaDestino,
                etiqueta = "Entrega a oficina destino",
                icono = "storefront",
                requiere = "oficina"
            },
            new
            {
                valor = ModosEscaneo.EntregaAlClienteEnOficina,
                etiqueta = "Entrega al cliente en oficina",
                icono = "how_to_reg",
                requiere = "oficina"
            },
            new
            {
                valor = ModosEscaneo.SalidaAReparto,
                etiqueta = "Salida a reparto",
